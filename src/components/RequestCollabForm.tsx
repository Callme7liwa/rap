...
export function RequestCollabForm({ artistId, artistName, onSuccess, onCancel }: RequestCollabFormProps) {
  ...
  const fetchArtists = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/artists?limit=1000`); // Get all artists
      if (!response.ok) {
  ...
  if (!canRequest && !limitData?.isArtist) {
    ...
    <Card>
      <form onSubmit={handleSubmit}>
        <CardHeader>
          ...
          <CardDescription>
            {limitData?.isArtist ? (
              <span>As an artist, you have <strong>unlimited</strong> collaboration requests</span>
            ) : (
