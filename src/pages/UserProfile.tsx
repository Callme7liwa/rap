...
export default function UserProfile() {
  ...
  const loadUserProfile = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_ENDPOINT}/user/profile`, {
        headers: {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formDataUpload.append('category', 'profile-pictures');
      formDataUpload.append('visibility', 'public');

      const uploadResponse = await fetch(`${import.meta.env.VITE_API_ENDPOINT}/uploads`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      // Extract URL from nested upload object or root level
      const imageUrl = uploadData.upload?.url || 
                       uploadData.upload?.fileUrl || 
                       uploadData.url || 
                       uploadData.fileUrl || 
                       uploadData.file_url || 
      ...
      if (!imageUrl) {
        console.error('❌ No URL found in response:', uploadData);
      ...
      console.log('💾 Auto-saving profile with new picture...');

      // Immediately save to database
      const saveResponse = await fetch(`${import.meta.env.VITE_API_ENDPOINT}/user/profile`, {
        method: 'PUT',
        headers: {
