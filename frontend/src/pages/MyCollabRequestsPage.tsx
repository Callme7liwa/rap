...
import { useCancelCollabRequest, useMyCollabRequests } from '@/hooks/useCollabRequests';
import { Loader2, Send, AlertCircle, Calendar } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useNavigate } from 'react-router-dom';

export function MyCollabRequestsPage() {
  const { data, isLoading, error } = useMyCollabRequests();
  const cancelMutation = useCancelCollabRequest();
        ...
        <CardContent>
          ...
          {limit?.canRequest ? (
            ...
            <Alert className="mt-4" variant="destructive">
              <AlertDescription>
                You've reached your monthly collab request limit.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
