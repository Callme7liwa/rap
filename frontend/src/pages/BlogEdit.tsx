...
import { getBlogPostForEdit, updateBlogPost } from '@/lib/blog-api';
...
export default function BlogEdit() {
  ...
  const handleSave = async (status: 'DRAFT' | 'PUBLISHED') => {
    ...
    try {
      ...
      const updatedPost = await updateBlogPost(slug, updatedData);
      ...
      setTimeout(() => {
        navigate(
          status === 'PUBLISHED'
            ? `/blog/${updatedPost.slug}`
            : `/blog/${updatedPost.slug}/edit`,
        );
      }, 500);
    } catch (error) {
