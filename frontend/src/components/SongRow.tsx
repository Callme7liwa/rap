...
export function SongRow({ song, index = 0, showArtwork = true }: SongRowProps) {
  ...
  return (
    <motion.div
      ...
      className="group flex items-center gap-4 p-3 rounded-none hover:bg-muted/50 transition-colors border border-transparent hover:border-primary/50"
    >
      <div className="font-display text-3xl text-muted-foreground/30 w-8 text-center flex-shrink-0">
        {index + 1}
      </div>
      <Link
        to={`/songs/${song.id}`}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="font-semibold uppercase tracking-wide truncate group-hover:text-primary transition-colors">
              {song.title}
            </h4>
            {isOwned && (
